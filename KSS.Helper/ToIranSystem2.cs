...
{
    public static class ConvertToIranSystem2
    {
        private static readonly Dictionary<byte, byte> CharachtersMapper_Group1 = new Dictionary<byte, byte>
    {
        {48 , 128}, // 0
        {49 , 129}, // 1
        {50 , 130}, // 2
        ...
        private static readonly Dictionary<byte, byte> CharachtersMapper_Group2 = new Dictionary<byte, byte>
        {
       {48,128},//
       {49,129},//
       {50,130},
       ...
       {144,240},//گ
        ...
        private static readonly Dictionary<byte, byte> CharachtersMapper_Group3 = new Dictionary<byte, byte>
        {
        ...
        private static readonly Dictionary<byte, byte> CharachtersMapper_Group4 = new Dictionary<byte, byte>
        {
            {48 , 128}, // 0
            {49 , 129}, // 1
            {50 , 130}, // 2
        ...
        public static string Get_Unicode_To_IranSystem(string Unicode_Text)
        {
            ...
            string readText = Unicode_Text;

            using (MemoryStream memoryStream = new MemoryStream())
            {
                memoryStream.Write(IS_Result.ToArray(), 0, IS_Result.ToArray().Length);

                memoryStream.Seek(0, SeekOrigin.Begin);

                byte[] dataToRead = new byte[IS_Result.ToArray().Length];
                int numberOfBytesRead = memoryStream.Read(dataToRead, 0, dataToRead.Length);
                readText = Encoding.Unicode.GetString(dataToRead);
            }
